...
file.copy("/w5home/bmoore/LiFang_scRNAseq/config.json", file.path(output, "config.json"))
config <- fromJSON(file.path(output, "config.json"))
output <- paste0(output, "/")
source("/w5home/bmoore/LiFang_scRNAseq/src/sc_pipeline_functions.R")
```
# read in counts
```{r read}
# loop
...
```{r normalize}
normalized_seurat_obj<- normalize_data(seurat.big)
